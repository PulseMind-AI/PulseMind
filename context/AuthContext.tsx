...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    console.log('Auth state changed:', { isAuthenticated, user: user?.username || null });
  ...
  const signup = async (username: string, email: string, city: string , password: string): Promise<void> => {
    try {
      ...
      console.log('Signup successful, user authenticated:', user.username);
    } catch (error: any) {
  ...
  const googleLogin = async (credential: string): Promise<void> => {
    try {
      ...
      console.log('Google login successful, user authenticated:', user.username);
    } catch (error: any) {
      const error_Message = error.response?.data?.error || 'Google login failed. Please try again.';
      console.error('Google login error:', error_Message);
      throw new Error(error_Message);
    }
  };
